...
export class MoegirlMCPServer {
     ...
     * 处理搜索萌娘百科
     */
    async handleSearchMoegirl(args) {
    ...
    async handleGetPage(args) {
        ...
        if (!pageid && !title) {
            throw new Error('必须提供 pageid 或 title 参数');
        }
        // 检查缓存
     ...
     * 处理获取页面段落
     */
    async handleGetPageSections(args) {
...
`;
     ...
     * 格式化页面内容（包含目录）
     */
    formatPageContentWithTOC(page, maxLength = 2000) {
        // 解析页面结构
...
`;
     ...
     * 格式化页面内容
     */
    formatPageContent(page, maxLength = 2000) {
        const content = page.cleaned_content || page.content;
        let text = `📖 ${page.title}
`;
        text += '='.repeat(page.title.length + 3) + '\n\n';
